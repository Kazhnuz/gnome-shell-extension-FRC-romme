...
Astro.prototype = {
    ...
    jd_to_french_revolutionary: function (jd) {
        var franciade_millenaire, franciade_seculaire, siecle, franciade, an, mois, decade, jour, adr, equinoxe;
	...
	jdCacheStamp = jd;
    	...
    	jd = jd - this.gregorian_to_jd (1792, 9, 22);
	
	franciade_millenaire = Math.floor( jd / 1460969 );
	jd = jd % 1460969;

	franciade_seculaire = Math.floor( jd / 146097 );
	jd = jd % 146097;

	if (jd < 109572) {
		siecle = Math.floor( jd / 36524 );
		jd = jd % 36524;
	} else {
		siecle = 3;
		jd = jd - 109572;
	...
	franciade = Math.floor( jd / 1461 );
	jd = ( jd % 1461 );
	
	if (jd < 1095) {
		an = Math.floor( jd / 365 );
		jd = jd % 365;
	} else {
		an = 3;
		jd = jd - 1095;
        ...
        an = (franciade_millenaire*4000) + (franciade_seculaire*400) + (siecle*100) + (franciade*4) + an + 1;
        mois = Math.floor((jd) / 30) + 1;
        jour = (jd) % 30;
        decade = Math.floor(jour / 10) + 1;
        jour = (jour % 10) + 1;

        this.cachestamp['FRC']=jdCacheStamp;
